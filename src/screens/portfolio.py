...
from utils.volatility import annualized_volatility_from_prices, calcular_retorno_carteira
import pandas as pd
...
def retorno_total_carteira(df, valor_inicial=1000, pesos=None):
    ...
    # Retornos ponderados
    retorno_ponderado = sum(p * df[ticker] for p, ticker in zip(pesos, df.index if isinstance(df, pd.Series) else df.columns))
